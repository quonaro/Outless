...
func NewJWTService(secretKey string, expiry time.Duration) (*JWTService, error) {
	if len(secretKey) < 32 {
		return nil, fmt.Errorf("jwt secret must be at least 32 characters, got %d", len(secretKey))
	}
	return &JWTService{secretKey: []byte(secretKey), expiry: expiry}, nil
}
