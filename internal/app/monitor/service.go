...
// Config controls monitor worker pool behavior.
type Config struct {
	Workers int
}
...
// NewService builds a monitor service with constructor injection.
...
func (s *Service) RunOnce(ctx context.Context) error {
	...
	if err := group.Wait(); err != nil {
		return fmt.Errorf("running monitor cycle: %w", err)
	}
...
func (s *Service) RunLoop(ctx context.Context, ticks <-chan struct{}) error {
	...
	for {
		...
		case <-ticks:
			...
			if err != nil {
				once.Do(func() {
					s.logger.Error("monitor cycle failed", slog.String("error", err.Error()))
				})
			}
		}
	}
