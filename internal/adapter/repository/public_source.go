...
func (publicSourceModel) TableName() string { return "public_sources" }

// PublicSourceRepository persists public sources using GORM over SQLite.
type PublicSourceRepository struct {
	db     *gorm.DB
	logger *slog.Logger
}

// NewPublicSourceRepository constructs a GORM-backed public source repository.
func NewPublicSourceRepository(db *gorm.DB, logger *slog.Logger) *PublicSourceRepository {
	return &PublicSourceRepository{db: db, logger: logger}
}

func (r *PublicSourceRepository) Create(ctx context.Context, source domain.PublicSource) error {
...
func (r *PublicSourceRepository) FindByID(ctx context.Context, id string) (domain.PublicSource, error) {
	var model publicSourceModel
	err := r.db.WithContext(ctx).Where("id = ?", id).First(&model).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return domain.PublicSource{}, fmt.Errorf("public source not found: %w", domain.ErrPublicSourceNotFound)
		}
...
func (r *PublicSourceRepository) List(ctx context.Context) ([]domain.PublicSource, error) {
...
func (r *PublicSourceRepository) Delete(ctx context.Context, id string) error {
	result := r.db.WithContext(ctx).Where("id = ?", id).Delete(&publicSourceModel{})
	if result.Error != nil {
